...
`;var copy_to_clipboard=__webpack_require__(20640);const CopyIcon=({onClick:onClick=(()=>{})})=>svg`
...
`} ${nodeStyles.verticalPadding}
  ...
  </div>`},CSSProperty=cssProperty=>{const{property:property,value:value,color:color}=cssProperty;let coloredSquare=null;switch(property){case"background":case"fill":case"border":case"box-shadow":case"color":coloredSquare=html`<span
        class="color-preview"
...
`,Footer=metadata=>{if(!metadata||!metadata.link||void 0===metadata.link||"undefined"===metadata.link)return null;const{link:link,timestamp:timestamp,fileName:fileName}=metadata;return html`<a
    class="figma-footer"
    target="_blank"
    rel="noopener"
    title="Open in Figma"
        ...
        "
          >
            ${Object.entries(this.__images).map((([nodeId,uri])=>{var _a;const node=ViewerMixin_classPrivateFieldGet(this,_getNodeById).call(this,nodeId);if(!node||!("absoluteBoundingBox"in node)||!(null===(_a=ViewerMixin_classPrivateFieldGet(this,_effectMargins))||void 0===_a?void 0:_a[node.id]))return null;const margin=ViewerMixin_classPrivateFieldGet(this,_effectMargins)[node.id];return html`
                ...
                " />
              `}))}
            ${this.selectedNode&&(({nodeSize:{x:x,y:y,width:width,height:height},offsetX:offsetX,offsetY:offsetY,reverseScale:reverseScale})=>html`
