...
export interface AlertProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   */
  variant?: keyof typeof ALERT_ENUM;
  /**
...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>((props, ref) => {
  const { className, children, variant = 'neutral', onClose, ...other } = props;
  const {
    theme: { alert },
  ...
  const typeStyle = alert[variant];
  const iconBaseStyle = alert.icon.base;
  const iconTypeStyle = alert.icon[variant];
